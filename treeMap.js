...
Tree.prototype.map = function (callback) {
  var startingNode = new Tree(callback(this.value));
  var cloning = function(newNode, node) {
    for(var i = 0; i < node.children.length; i++) {
      var child = new Tree(callback(node.children[i].value));
      newNode.children.push(child);
      cloning(newNode.children[i], node.children[i]);
    }
  };
  cloning(startingNode, this);
  return startingNode;
};
...
console.log('this is original tree: ', input);